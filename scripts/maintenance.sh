...
SKIP_RPM_OSTREE=false
SKIP_FLATPAK=false
DRY_RUN=false

usage() {
  cat <<'USAGE'
Uso: maintenance.sh [opcoes]
...
Opcoes:
  ...
  --skip-rpm-ostree       Pula etapa rpm-ostree
  --skip-flatpak          Pula etapa Flatpak
...
if [[ "$SKIP_RPM_OSTREE" == false ]]; then
  ...
  log "Etapa rpm-ostree ignorada (--skip-rpm-ostree)."
fi
...
if [[ "$SKIP_FLATPAK" == false ]]; then
  ...
  log "Etapa Flatpak ignorada (--skip-flatpak)."
fi
...
if [[ "$SKIP_RPM_OSTREE" == false ]] && command_exists rpm-ostree; then
