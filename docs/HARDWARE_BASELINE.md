...
- Kernel arguments keep `amd_pstate=active` for Zen 3 frequency scaling, `preempt=full` for dynamic low-latency desktop preemption, enable KVM nested virtualization support (`kvm_amd.nested=1`) for libvirt, and enforce CIS/KSPP exploit mitigations (`slab_nomerge`, `vsyscall=none`, `randomize_kstack_offset=on`).
